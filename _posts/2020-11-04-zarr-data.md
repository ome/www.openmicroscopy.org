...
<iframe style="width: 100%; height: 500px" name="vizarr" src="https://hms-dbmi.github.io/vizarr/v0.1/?source=https://livingobjects.ebi.ac.uk/idr/zarr/v0.1/9836831.zarr">
</iframe>
            ...
            <table>
                ...
                {% for row in site.data.zarr_data_2020-11-04.images %}
                    <tr>
                        ...
                        <td>{{ row.T }} </td>
                        <td>
                            <a title="Open in viewer above" target='vizarr' href="https://hms-dbmi.github.io/vizarr/v0.1/?source=https://livingobjects.ebi.ac.uk/idr/zarr/v0.1/{{ row.Image }}.zarr">
                                view
                            </a>
                        </td>
                        <td>
                            <button class="hollow button" title="S3 endpoint. Copy to clipboard" onclick="copyTextToClipboard('https://livingobjects.ebi.ac.uk/idr/zarr/v0.1/{{ row.Image }}.zarr')">
                                copy
                            </button>
                            {% if row.Image == '6001247' %}
                                <br>(with <a href="https://livingobjects.ebi.ac.uk/idr/zarr/v0.1/{{ row.Image }}.zarr/labels/">labels</a>)
                            {% endif %}
                        </td>
                    </tr>
                {% endfor %}
